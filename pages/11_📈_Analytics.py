...
if not candidates:
    ...
    ⚠️ No data yet. Complete <strong>Resume Screening</strong> to see analytics.</div>""",
...
sem_scores = [safe_sem(c)   for c in candidates]
...
for (label, count), color in zip(buckets.items(), bar_colors):
    ...
    <div style='display:flex;flex-direction:column;align-items:center;gap:6px;flex:1;'>
        ...
        <div style='width:100%;max-width:60px;background:{color};border-radius:4px 4px 0 0;
        height:{h}px;min-height:{"4px" if count>0 else "0"};transition:height 0.5s ease;'></div>
        <div style='font-family:DM Sans;font-size:11px;color:rgba(232,230,240,0.5);
        text-align:center;'>{label}</div>
        <div style='font-family:DM Sans;font-size:10px;color:rgba(232,230,240,0.3);'>{pct:.0f}%</div>
    </div>"""

st.markdown(f"""
<div style='background:#0d0d14;border:1px solid rgba(255,255,255,0.07);border-radius:12px;
padding:24px 32px;'>
...
with col2:
    ...
    for c in sorted(candidates, key=safe_score, reverse=True)[:8]:
        ...
        padding:6px 10px;background:#13131f;border-radius:6px;'>
            ...
            <div style='font-family:DM Sans;font-size:11px;color:{diff_color};width:40px;text-align:right;'>
            {("+" if diff >= 0 else "")}{diff:.0f}</div>
        </div>""", unsafe_allow_html=True)
    st.markdown("""<div style='font-family:DM Sans;font-size:11px;color:rgba(232,230,240,0.3);
    margin-top:6px;'><span style='color:#a78bfa;'>■</span> AI  
    <span style='color:#06b6d4;margin-left:8px;'>■</span> Semantic  
    <span style='color:rgba(232,230,240,0.3);margin-left:8px;'>Δ = difference</span></div>""",
    unsafe_allow_html=True)
...
total_pipe = sum(stage_counts.values()) if stage_counts else len(candidates)

funnel_html = "<div style='background:#0d0d14;border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:24px 32px;'>"
max_c = max(stage_counts.values()) if stage_counts and max(stage_counts.values())>0 else max(len(candidates),1)

for stage in STAGES:
    count = stage_counts.get(stage, 0)
    if stage == "Screened" and count == 0:
        count = len(candidates)
    color = STAGE_COLORS[stage]
    pct = count/total_pipe*100 if total_pipe else 0
    ...
    <div style='display:flex;align-items:center;gap:16px;margin-bottom:14px;'>
        ...
        color:{color};width:28px;'>{count}</div>
        <div style='font-family:DM Sans;font-size:11px;color:rgba(232,230,240,0.3);
        width:36px;'>{pct:.0f}%</div>
    </div>"""
funnel_html += "</div>"
...
top5 = sorted(candidates, key=safe_score, reverse=True)[:5]
medals = ["🥇","🥈","🥉","④","⑤"]
...
for i, cand in enumerate(top5):
    ...
    <div style='padding:8px 0;'>
        <div style='font-family:DM Sans;font-size:13px;font-weight:500;color:#e8e6f0;'>{cand['name']}</div>
        <div style='font-family:DM Sans;font-size:11px;color:rgba(232,230,240,0.35);margin-top:2px;'>{cand.get('email','')}</div>
    </div>""", unsafe_allow_html=True)
...
insights = []
if avg_score < 50:
    insights.append(("⚠️", "Low average score", f"Average is {avg_score:.0f}%. Consider broadening the candidate search or revising JD requirements.", "#fbbf24"))
if shortlisted/len(candidates) > 0.5 if candidates else False:
    insights.append(("✅", "High shortlist rate", f"{shortlisted/len(candidates)*100:.0f}% shortlisted — strong candidate pool for this role.", "#22c55e"))
if shortlisted == 0:
...
if not insights:
    insights.append(("✅", "Pipeline looks healthy", f"{len(candidates)} candidates screened, {shortlisted} shortlisted, avg score {avg_score:.0f}%.", "#22c55e"))
...
for icon, title, desc, color in insights:
    ...
    border-radius:0 8px 8px 0;padding:14px 16px;margin-bottom:10px;display:flex;gap:12px;'>
        ...
        <div>
            <div style='font-family:Syne,sans-serif;font-size:13px;font-weight:700;
            color:#e8e6f0;margin-bottom:4px;'>{title}</div>
            <div style='font-family:DM Sans;font-size:12px;color:rgba(232,230,240,0.55);
