...
if candidate:
    st.markdown(f"<div style='background:rgba(34,197,94,0.06);border:1px solid rgba(34,197,94,0.15);border-radius:8px;padding:12px 16px;font-family:DM Sans;font-size:13px;color:#22c55e;margin-bottom:16px;'>✓ Candidate: <strong>{candidate['name']}</strong> · AI Score: {candidate['ai_score']:.0f}%</div>", unsafe_allow_html=True)
...
if kit:
    ...
    cats = [("technical_questions",tabs[0]),("behavioral_questions",tabs[1]),("situational_questions",tabs[2]),("culture_fit_questions",tabs[3]),("red_flag_probing_questions",tabs[4])]
    for key, tab in cats:
        with tab:
    ...
    with tabs[5]:
        if rubric:
            st.json(rubric)
    st.markdown("<br>", unsafe_allow_html=True)
    st.download_button("📥 Download Interview Kit (JSON)", data=json.dumps({"questions":kit,"rubric":rubric},indent=2),
        file_name=f"interview_kit_{candidate['name'].replace(' ','_')}.json", mime="application/json", use_container_width=True)
